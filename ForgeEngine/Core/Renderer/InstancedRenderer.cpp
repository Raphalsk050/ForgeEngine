...
{
    extern bool PerformCulling(int entityID, const glm::mat4& transform);
    
    extern struct Renderer3DData s_Data;
    ...
    void InstancedRenderer::Init()
    {
        ...
        if (!m_InstanceBuffer)
        {
            FENGINE_CORE_ERROR("Failed to create instance buffer!");
            return;
        }
                                             ...
                                             const std::vector<int>& entityIDs)
    {
        ...
        {
            FENGINE_CORE_ERROR("DrawInstancedMesh: Invalid mesh, buffer, or shader!");
            return;
        }
        ...
        if (!instancedVAO)
        {
            FENGINE_CORE_ERROR("Failed to create or get instanced VAO!");
            return;
        }
...
layout(location = 3) in mat4 a_InstanceMatrix;     // locations 3,4,5,6
layout(location = 7) in vec4 a_InstanceColor;      // location 7
layout(location = 8) in vec4 a_InstanceCustomData; // location 8
...
void main()
{
    ...
    o_Color = vec4(finalColor, finalAlbedo.a);
        ...
        try 
        {
            ...
            {
                FENGINE_CORE_ERROR("Failed to create instanced shader!");
            }
            else
            {
        ...
        {
            FENGINE_CORE_ERROR("Exception creating instanced shader: {}", e.what());
        }
    }
                                               ...
                                               const std::vector<int>& entityIDs)
    {
        ...
        for (size_t i = 0; i < transforms.size(); ++i)
        {
            ...
            {
                FENGINE_CORE_WARN("Reached maximum instance limit: {}", MAX_INSTANCES);
                break;
            }
    ...
    Ref<VertexArray> InstancedRenderer::GetOrCreateInstancedVAO(Ref<Mesh> mesh)
    {
        ...
        if (!instancedVAO)
        {
            FENGINE_CORE_ERROR("Failed to create instanced VAO!");
            return nullptr;
        }
        ...
        {
            FENGINE_CORE_ERROR("Mesh has no vertex buffers!");
            return nullptr;
        }
    ...
    void InstancedRenderer::SetupInstanceAttributes(Ref<VertexArray> vao)
    {
        ...
        uint32_t index = 3; // Start after mesh attributes (0,1,2)
    ...
    {
        s_Data.DefaultMaterial->GetAlbedoMap()->Bind(0);
        ...
        s_Data.Stats.VertexCount += mesh->GetVertexCount() * instanceCount;
        s_Data.Stats.IndexCount += mesh->GetIndexCount() * instanceCount;
        s_Data.Stats.VisibleMeshCount += instanceCount;
    }
